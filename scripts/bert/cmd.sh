...
if [ "$NP" = "256" ]; then
    ...
    #export NUMSTEPS=7038
    #export USE_SA=1
    #LOGINTERVAL=10 BS=65536 ACC=8 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.006 WARMUP_RATIO=0.2843 bash mul-hvd.sh
    #exit
    ###### DEBUG ########

    #LOGINTERVAL=1 BS=32768 ACC=2 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.005 WARMUP_RATIO=0.2 bash mul-hvd.sh
    export NUMSTEPS=7038
    #LOGINTERVAL=10 BS=65536 ACC=2 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.003 WARMUP_RATIO=0.45 bash mul-hvd.sh
    #LOGINTERVAL=10 BS=65536 ACC=4 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.006 WARMUP_RATIO=0.2843 bash mul-hvd.sh
    export DTYPE='float16'
    #LOGINTERVAL=10 BS=65536 ACC=4 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.006 WARMUP_RATIO=0.3 bash mul-hvd.sh
    #LOGINTERVAL=10 BS=65536 ACC=4 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.006 WARMUP_RATIO=0.2843 bash mul-hvd.sh
    ...
    #LOGINTERVAL=10 BS=65536 ACC=4 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.006 WARMUP_RATIO=0.2843 bash mul-hvd.sh
    #export NUMSTEPS=21
    LOGINTERVAL=10 BS=65536 ACC=2 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.006 WARMUP_RATIO=0.2843 bash mul-hvd.sh
    echo 'DONE phase1'
elif [ "$NP" = "1" ]; then
    export DTYPE='float32'
    ...
    export DTYPE='float32'
    #LOGINTERVAL=10 BS=256 ACC=1 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.005 WARMUP_RATIO=0.2 bash mul-hvd.sh
    LOGINTERVAL=1 BS=64 ACC=1 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.005 WARMUP_RATIO=0.2 bash mul-hvd.sh
else
    export DTYPE='float32'
    export BS=$NP
    LOGINTERVAL=10 ACC=1 MAX_SEQ_LENGTH=128 MAX_PREDICTIONS_PER_SEQ=20 LR=0.005 WARMUP_RATIO=0.2 bash mul-hvd.sh
fi
#################################################################
...
if [ "$NP" = "512" ]; then
    #DTYPE='float32'
    #LOGINTERVAL=1 BS=32768 ACC=16 MAX_SEQ_LENGTH=512 MAX_PREDICTIONS_PER_SEQ=80 LR=0.004 WARMUP_RATIO=0.128 bash mul-hvd.sh
    # export NUMSTEPS=782
    #LOGINTERVAL=1 BS=65536 ACC=16 MAX_SEQ_LENGTH=512 MAX_PREDICTIONS_PER_SEQ=80 LR=0.005 WARMUP_RATIO=0.2 bash mul-hvd.sh
    #LOGINTERVAL=1 BS=32768 ACC=8 MAX_SEQ_LENGTH=512 MAX_PREDICTIONS_PER_SEQ=80 LR=0.004 WARMUP_RATIO=0.128 bash mul-hvd.sh
    DTYPE='float16'
    export CYCLETIME=85
    BS=33280 ACC=5 MAX_SEQ_LENGTH=512 MAX_PREDICTIONS_PER_SEQ=80 LR=0.004 WARMUP_RATIO=0.4 bash mul-hvd.sh
    # BS=33792 ACC=6 MAX_SEQ_LENGTH=512 MAX_PREDICTIONS_PER_SEQ=80 LR=0.004 WARMUP_RATIO=0.4 bash mul-hvd.sh
    # BS=32768 ACC=8 MAX_SEQ_LENGTH=512 MAX_PREDICTIONS_PER_SEQ=80 LR=0.004 WARMUP_RATIO=0.4 bash mul-hvd.sh
    echo 'DONE phase2'
elif [ "$NP" = "8" ]; then
    DTYPE='float32'
    LOGINTERVAL=10 BS=32 ACC=1 MAX_SEQ_LENGTH=512 MAX_PREDICTIONS_PER_SEQ=80 LR=0.004 WARMUP_RATIO=0.128 bash mul-hvd.sh
fi
