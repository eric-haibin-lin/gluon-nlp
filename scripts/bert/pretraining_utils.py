...
def get_pretrain_data_npz(data, batch_size, num_ctxes,
    ...
    Returns
    -------
    dataloader : nlp.data.DatasetLoader, or None if not enough files
    """
    num_files = len(nlp.utils.glob(data))
    logging.info('%d files are found.', num_files)
    # Not enough files
...
def log_noacc(begin_time, running_num_tks, running_mlm_loss, running_nsp_loss, step_num,
    ...
    avg_running_mlm_loss = hvd.allreduce(running_mlm_loss, name='avg_running_mlm_loss')
    logging.info('[step {}]\tmlm_loss={:7.5f}\tnsp_loss={:5.2f}\tthroughput={:.1f}K tks/s\tlr={:.7f} time={:.2f}, latency={:.1f} ms/batch, avg_mlm={:.5f}'
                 .format(step_num, running_mlm_loss.asscalar(), running_nsp_loss.asscalar(),
                         throughput.asscalar(), lr, duration, duration*1000/log_interval, avg_running_mlm_loss.asscalar()))
